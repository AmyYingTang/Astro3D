...
function MessierObject({ obj, index }) {
  ...
  const InfoPanel = ({ visible = true }) => (
    <Html position={[0, imageUrl ? 0.25 : 0.15, 0]} center distanceFactor={10} pointerEvents="none">
      <div style={{
        opacity: visible ? 1 : 0,               // ✅ 用透明度控制显示
  ...
  if (imageUrl && !loading) {
    return (
      <group position={[x, y, z]} renderOrder={9999}>
        <ImageSprite 
          imageUrl={imageUrl} 
          size={0.3}
          onClick={handleClick}
        ...
        {hovered && <InfoPanel visible={hovered} />}
      </group>
    );
  }
