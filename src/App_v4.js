...
function MessierObject({ obj, index }) {
  ...
  console.log(`${obj.name} hovered:`, hovered);
    ...
    return (
      <group position={[x, y, z]}>
        <ImageSprite 
          ...
          size={0.3}
          onPointerEnter={() => {
            console.log(`Hovering ${obj.name} (with image)`);
            setHovered(true);
          }}
          onPointerLeave={() => {
            console.log(`Left ${obj.name} (with image)`);
            setHovered(false);
          }}
        />
        <Text
...
// === 图片精灵组件 ===
function ImageSprite({ imageUrl, size = 0.3, onPointerEnter, onPointerLeave }) {
